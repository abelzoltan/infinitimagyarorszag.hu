...
if(!isset($postData["contactType"]) AND isset($_GET["tesztvezetes"]) AND $_GET["tesztvezetes"]) { $postData["contactType"] = "tesztvezetes"; }

if($formType == "general") 
{ 
	?>
	<script>
		function modelChange(e) { $("[name=modelName]").val($(e).find("option:selected").text()); }
	</script>
	<?php
} 
?>
		...
		<div class="parsys col1-par">
			<div class="form section">
				<div class="c_019">
					...
					<p style="font-size: 1.2em; font-weight: bold; text-align: center; margin-bottom: 0;">Köszönjük érdeklődését modellünk iránt! Kérjük, töltse ki az alábbi adatlapot, hogy felvehessük Önnel a kapcsolatot. Érdeklődését a lehető legrövidebb időn belül feldolgozzuk.</p>
					<form method="post" action="<?php echo PATH_WEB; ?>kapcsolatfelvetel" target="_self" id="contactForm">
						...
						{
							?>
							<input type="hidden" name="model" value="<?php echo $GLOBALS["URL"]->routes[0]; ?>">
							<input type="hidden" name="modelName" value="<?php echo $VIEW["title"]; ?>">
							<?php
						}
						elseif($formType == "oldCar") { ?><input type="hidden" name="modelURL" value="<?php echo $car["url"]; ?>"><?php } 
						?>
						<fieldset>
							<div class="parsys formBuilder">
								...
								<div class="textfield section">
									<div class="form-group">
										<label>Kapcsolatfelvétel típusa*</label>
										<div class="form-group-container my-infiniti-radio">
											<label><input type="radio" name="contactType" value="ajanlatkeres" <?php if(!isset($postData["contactType"]) OR empty($postData["contactType"]) OR $postData["contactType"] == "ajanlatkeres") { ?>checked<?php } ?>><span>Ajánlatkérés</span></label>
											<label><input type="radio" name="contactType" value="tesztvezetes" id="my-infiniti-radio-t" <?php if($postData["contactType"] == "tesztvezetes") { ?>checked<?php } ?>><span>Tesztvezetés</span></label>
										</div>
									</div>
								</div>
								<div class="textfield section">
